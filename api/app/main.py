...
# Path for csv file containing cities coordinate
csv_name = '/opt/dev/genetic-tsp/api/app/algeria-cities.csv'
...
class GA(object):
    ...
    def tournament_select(self, population, cities):
        '''
        RoutePop() --> Route()
    ...
    def evolve_population(self, init_pop, cities):
        '''
        RoutePop() --> RoutePop()
        ...
        for x in range(elitismOffset,descendant_pop.size):
            # two parents:
            tournament_parent1 = self.tournament_select(init_pop, cities)
            tournament_parent2 = self.tournament_select(init_pop, cities)
...
class TspAPP(object):
    ...
    """
    def __init__(self,n_generations,pop_size, websocket=None):
        '''
    ...
    async def GA_loop(self):
        ...
        for x in range(1,self.n_generations):
            ...
            the_population = GA().evolve_population(the_population, self.cities)
